...
<svelte:head>
	...
	<meta name="description" content="Free online JSON formatter, validator, and minifier with syntax highlighting. Validate JSON and get detailed error messages." />
</svelte:head>
...
<div class="max-w-6xl mx-auto">
		...
		<div class="mb-4">
			...
			{:else}
				<div class="alert alert-error" role="alert">
					...
					<button class="btn btn-sm btn-ghost" onclick={() => { isValid = null; error = ''; }} aria-label="Dismiss">
						&times;
					</button>
				</div>
			{/if}
	...
	<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
		...
		<div class="form-control">
			...
			></textarea>
			<div class="mt-2">
				<button class="btn btn-xs btn-ghost" onclick={loadSampleJSON}>
					Load sample JSON
				</button>
			</div>
		</div>
		...
		<div class="form-control">
			...
			<div class="relative">
				{#if highlightedOutput}
					...
					<pre class="h-80 overflow-auto rounded-lg border border-base-300 bg-base-200 p-4 font-mono text-sm"><code class="language-json">{@html highlightedOutput}</code></pre>
				{:else}
					<textarea
						id="json-output"
						class="textarea textarea-bordered h-80 font-mono text-sm bg-base-200 w-full"
	...
	<section class="mt-12 prose prose-sm max-w-none">
		...
		<p class="text-base-content/70">
			...
			This tool helps you format messy JSON into readable, properly indented structure.
		</p>
