<script lang="ts">
	...
	async function handleCopyHeader() {
		...
		if (success) {
			...
			setTimeout(() => { copyHeaderSuccess = false; }, 2000);
		}
	}
	...
	async function handleCopyPayload() {
		...
		if (success) {
			...
			setTimeout(() => { copyPayloadSuccess = false; }, 2000);
		}
	}
	...
	async function handleCopyToken() {
		...
		if (success) {
			...
			setTimeout(() => { copyTokenSuccess = false; }, 2000);
		}
	}
	...
	function loadSampleToken() {
		...
		tokenInput = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyLCJleHAiOjE1MTYyMzkwMjJ9.4Adcj3UFYzPUVaVF43FmMab6RlaQD8A9V8wFzzht-KQ';
		handleDecode();
	}
</script>
...
<svelte:head>
	...
	<meta name="description" content="Free online JWT decoder and encoder. Decode, verify, and create JSON Web Tokens with HS256 signature support." />
</svelte:head>
	...
	{#if activeTab === 'decode'}
		...
		{#if decodeError}
			<div class="alert alert-error mb-6" role="alert">
				...
				<button class="btn btn-sm btn-ghost" onclick={() => decodeError = ''} aria-label="Dismiss error">
					&times;
				</button>
			</div>
		{/if}
		...
		<fieldset class="fieldset w-full mb-6">
			...
			></textarea>
			<div class="mt-2">
				<button class="btn btn-xs btn-ghost" onclick={loadSampleToken}>
					Load sample token
				</button>
			</div>
		</fieldset>
							...
							<div>
								...
								{#if validationResult.expiresAt}
									<span class="block text-sm">
										Expires: {formatDate(validationResult.expiresAt)} ({getTimeRemaining(validationResult.expiresAt)})
									</span>
								{/if}
							</div>
						</div>
			...
			<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
				...
				<fieldset class="fieldset w-full">
					...
					<div class="relative">
						...
						<pre class="h-48 overflow-auto rounded-lg border border-base-300 bg-base-200 p-4 font-mono text-sm"><code class="language-json">{@html headerHighlighted}</code></pre>
						<button
				...
				<fieldset class="fieldset w-full">
					...
					<div class="relative">
						<!-- Safe to use {@html} here because:
							 ...
							 2. JSON.stringify escapes special characters
							 3. Prism.highlight only wraps tokens in <span> elements -->
						<pre class="h-48 overflow-auto rounded-lg border border-base-300 bg-base-200 p-4 font-mono text-sm"><code class="language-json">{@html payloadHighlighted}</code></pre>
						<button
							class="btn btn-sm btn-ghost absolute top-2 right-2"
	...
	{#if activeTab === 'encode'}
		...
		{#if encodeError}
			<div class="alert alert-error mb-6" role="alert">
				...
				<button class="btn btn-sm btn-ghost" onclick={() => encodeError = ''} aria-label="Dismiss error">
					&times;
				</button>
			</div>
		{/if}
		...
		<div class="form-control mb-6">
			...
			/>
			<div class="label">
				<span class="label-text-alt text-warning">Never share your secret key. This tool runs entirely in your browser.</span>
			</div>
		</div>
